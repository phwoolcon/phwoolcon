/**
 * EventTarget support
 * @see https://github.com/Robdel12/DropKick/blob/d1b348563d0e4bf775b90be1d275bc9513caf0d3/lib/polyfills/eventtarget.js
 */
(function () {
    ...
    var expando = "__events";
    /* own property for storing listeners */
    var flag = "__immediateStopped";
    /* stopImmediatePropagation flag */
    ...
    Event.prototype.stopImmediatePropagation = function () {
        this[flag] = this.cancelBubble = true;
    };

    var decorate = function (e, listenerNode) { /* improve event properties */
        e.timeStamp = +new Date();
        if (!e.target) {
            e.target = e.srcElement || listenerNode;
        }
    ...
    var indexOf = function (data, listener, useCapture) { /* return an index of an existing listener */
        for (var i = 0; i < data.length; i++) {
            var item = data[i];
            if (item.useCapture == useCapture && item.listener == listener) {
                return i;
            }
        }
        return -1;
    ...
    var runListeners = function (event, nodes, phase) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            for (var j = 0; j < data.length; j++) { /* get list of relevant listeners */
                var item = data[j];
                if (item.useCapture && phase == Event.BUBBLING_PHASE) {
                    continue;
                }
                if (!item.useCapture && phase == Event.CAPTURING_PHASE) {
                    continue;
                }
    ...
    var handler = function (event) {
        ...
        if (ancestors.length) { /* capture */
            if (runListeners(event, ancestors, Event.CAPTURING_PHASE)) {
                return event.returnValue;
            }
        }

        /* at target */
        if (runListeners(event, [event.target], Event.AT_TARGET)) {
            return event.returnValue;
        }

        if (ancestors.length && event.bubbles !== false) { /* bubble */
            ancestors.reverse();
            if (runListeners(event, ancestors, Event.BUBBLING_PHASE)) {
                return event.returnValue;
            }
        }

        event.stopPropagation();
        /* do not process natively */
        return event.returnValue;
    };
    ...
    var proto = {
        addEventListener: function (type, listener, useCapture) {
            ...
            if (indexOf(data, listener, useCapture) > -1) {
                return;
            }
            /* already added */
            ...
            if (!count) {
                this.attachEvent("on" + type, storage._handler);
            }
            /* first: add native listener */
        },
        ...
        removeEventListener: function (type, listener, useCapture) {
            ...
            data.splice(index, 1);
            if (!data.length) {
                this.detachEvent("on" + type, this[expando]._handler);
            }
            /* last: remove native listener */
        },
    ...
    while (todo.length) {
        var parent = todo.pop();
        for (var p in proto) {
            parent.prototype[p] = proto[p];
        }
    }
})();
/**
 * CustomEvent Support
 * @see https://github.com/Robdel12/DropKick/blob/d1b348563d0e4bf775b90be1d275bc9513caf0d3/lib/polyfills/customevent.js
 */
(function () {
